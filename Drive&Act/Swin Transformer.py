...
class MLP(nn.Module):
    ...
    def __init__(self, input_size, hidden_size=None, output_size=None, p=0.):
        super().__init__()
...
def window_reverse(windows, window_size, B, D, H, W):
    ...
    # Parameters:
    # window s: (B*num_windows, window_size, window_size, C)
    # window_size : (D, H, W)
    # B : Batch size
    # D : Number of frames
    # H : Height of image
...
class WindowAttention3D(nn.Module):
    ...
    # window_size : Frame (temporal length), height and width of the window (Wd, Wh, Ww).
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None, attn_p=0., proj_p=0.):
        super().__init__()
        self.dim = dim
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1) * (2 * window_size[2] - 1), num_heads))  # (2*Wd-1 * 2*Wh-1 * 2*Ww-1, num_heads)
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            nW = mask.shape[0]  # (num_windows, N, N)
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(2,7,7), shift_size=(0,0,0),
        ...
        self.attn = WindowAttention3D(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_p=attn_drop, proj_p=drop)
